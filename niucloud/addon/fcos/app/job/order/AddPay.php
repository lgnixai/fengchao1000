...
// | Author: addon888
// +----------------------------------------------------------------------
namespace addon\fcos\app\job\order;
...
use addon\fcos\app\model\order\Order;
...
//                ['create_time', '<=', time()-60]
            ...
            if(!$list->isEmpty()){
                foreach($list as $v){
                    ...
                    if(!Cache::get('fcos_order_add_'.$v['order_id'])){
                         (new NoticeService())->send($data['site_id'], 'fcos_order_add', ['order_id' => $data['order_id']]);
                        //更改通知次数
                        ...
                        Cache::set('fcos_order_add_'.$v['order_id'], '补差价通知', 60*60*23);
                    }
                }
            }
