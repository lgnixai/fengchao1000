...
use addon\fcos\app\model\fenxiao\FenxiaoOrder;
use addon\fcos\app\model\fenxiao\FenxiaoMember;
...
class OrderFinishService extends BaseApiService
{
    ...
    public function orderFinish($orderInfo)
    {
        try{
            //进行成长值发放
            $this->sendGrowth($orderInfo['site_id'], $orderInfo['member_id'], 'fcos_order', ['from_type' => 'fcos_order']);
            //积分发放
            $this->sendPoint($orderInfo['site_id'], $orderInfo['member_id'], 'fcos_order', ['from_type' => 'fcos_order']);
            //会员等级激励发放
    ...
    public function sendLevel($site_id, $member_id)
    {
        ...
        if ($member_info['member_level'] && !empty($member_info['member_level']['level_benefits'])) {
            ...
            foreach ($level_benefits as $k => $v) {
                if ($k == 'fcos_fenxiao' && $v['is_use'] == 1 && $v['expand'] > 0) {
