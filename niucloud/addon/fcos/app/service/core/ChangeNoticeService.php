...
use addon\fcos\app\model\shop_order\ShopOrder;
 ...
 * 易达接口对接
 * Class ConfigService
 * @package addon\fcos\service\core\yida
 */
    ...
    public function changeOrderStatus($data)
    {
        ...
        try {
            ...
            if ($data['contextObj']['ydOrderStatus'] == 11 || $data['contextObj']['ydOrderStatus'] == 2) {
                ...
                (new NoticeService())->send($orderInfo['site_id'], 'fcos_order_pick', ['order_id' => $orderInfo['order_id']]);
            }
            //订单完成时候完成系统订单  易达3 完成
            if ($data['contextObj']['ydOrderStatus'] == 3) {
                $orderInfo = $this->orderModel->where(['order_id' => $deliveryInfo['order_id']])->findOrEmpty();
                ...
                (new NoticeService())->send($orderInfo['site_id'], 'fcos_order_sign', ['order_id' => $orderInfo['order_id']]);
        ...
        try {
            ...
            if($price_add>0){
                ...
                if($addInfo->isEmpty()){
                    ...
                    (new NoticeService())->send($orderInfo['site_id'], 'fcos_order_add', ['order_id' => $orderInfo['order_id']]);
                }
            }
            Db::commit();
            return Response::create(['msg' => '接受成功', 'code' => 200, 'success' => true], 'json', 200);
