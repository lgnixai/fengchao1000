...
namespace addon\fcos\app\model\order;
...
//        return OrderDict::getType()[$data['order_type']] ?? '';
    ...
    {
        if(isset($data['taker_province'])&&$data['taker_province']=='') return '';
        $res=(new SysArea())->where(['id'=>$data['taker_province']])->findOrEmpty();
        return $res['name'];
    }
    public function getCityNameAttr($value, $data)
    {
        if(isset($data['taker_city'])&&$data['taker_city']=='') return '';
        $res=(new SysArea())->where(['id'=>$data['taker_city']])->findOrEmpty();
        return $res['name'];
    }
    public function getDistrictNameAttr($value, $data)
    {
        if(isset($data['taker_district'])&&$data['taker_district']=='') return '';
        $res=(new SysArea())->where(['id'=>$data['taker_district']])->findOrEmpty();
        return $res['name'];
    }
}
