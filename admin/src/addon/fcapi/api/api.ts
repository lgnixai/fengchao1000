...
export function getApiNameList() {
    return request.get(`fcapi/fake/list`)
}

export function getApiData(site_id:number,id:number) {
    return request.get(`fcapi/fake/${site_id}/${id}`)
}
/*****沙盒相关****/


export function sendSandBox(url:string,params: Record<string, any>) {
    return request.post(url, params, { showErrorMessage: true, showSuccessMessage: true })
}
