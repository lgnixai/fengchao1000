...
import { getApiKeyById, sendSandBox, getApiNameList, getApiData, getApiDomain } from '@/addon/fcapi/api/api'
    ...
    await formEl.validate(async (valid) => {
        if (valid) {
            ...
            sendSandBox(formData.url, data).then(res => {
                // router.push('/fcos/order/delivery/company')
                repeat.value = false
...
const back = () => {
    router.push({ path: '/fcapi/site/list' })
}
</script>
