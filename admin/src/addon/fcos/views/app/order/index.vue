<template>
  <div class="main-container">
    <el-card class="box-card !border-none" shadow="never">
          ...
          <el-table-column prop="order_code" :label="t('orderCode')" min-width="120">
            <template #default="{ row }">
              <el-tooltip
                  class="item"
                  effect="dark"
                  :content="row.order_code"
                  placement="top"
              >
                <span> {{ formatOrderCode(row.order_code) }}</span>
              </el-tooltip>
            </template>
          </el-table-column>
          <el-table-column prop="user_price.Weight" :label="t('weight')" min-width="120"/>


          <el-table-column prop="user_price.Cost" :label="t('Cost')" min-width="120"/>
          <el-table-column prop="user_price.InsureAmount" :label="t('InsureAmount')" min-width="120"/>
          <el-table-column prop="user_price.PackageFee" :label="t('PackageFee')" min-width="120"/>
          <el-table-column prop="user_price.OverFee" :label="t('OverFee')" min-width="120"/>
          <el-table-column prop="user_price.OtherFee" :label="t('OtherFee')" min-width="120"/>
          <el-table-column prop="user_price.TotalFee" :label="t('TotalFee')" min-width="120"/>
      ...
      <export-sure ref="exportSureDialog" :show="flag" type="fcos_order" :searchParam="orderTable.searchParam" @close="handleClose" />
...
import {t} from '@/lang'
import {getOrderList, } from '@/addon/fcos/api/order'
...
const detailEvent = (data: any) => {
  router.push('/fcos/order/detail?order_id=' + data.id)
}
