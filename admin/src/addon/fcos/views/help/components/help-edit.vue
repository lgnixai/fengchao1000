<template>
    <el-dialog v-model="showDialog" :title="formData.id ? t('updateHelp') : t('addHelp')" width="50%" class="diy-dialog-wrap"
...
import { addHelp, editHelp, getHelpInfo, } from '@/addon/fcos/api/help'
...
const confirm = async (formEl: FormInstance | undefined) => {
    ...
    let save = formData.id ? editHelp : addHelp
...
const setFormData = async (row: any = null) => {
    ...
    loading.value = true
    if(row){
        const data = await (await getHelpInfo(row.id)).data
